...
function generateTables() {
    ...
    // Generate table for current resource allocation
    ...
    // Generate table for maximum resource requirements
    ...
    // Show the "Run Banker's Algorithm" button
...
function generateTable(tableId, numRows, numCols, tableName) {
    ...
    const resourceRow = document.createElement('tr'); // Create a new row for resource numbers
    resourceRow.innerHTML = '<th></th>'; // Empty cell for the process numbers column
    for (let j = 0; j < numCols; j++) {
        resourceRow.innerHTML += `<th>R${j}</th>`; // Display resource numbers
    }
    tableHead.appendChild(resourceRow); // Append the resource row to the table header
    ...
    for (let i = 0; i < numRows; i++) {
        ...
        row.innerHTML = `<td>Process ${i + 1}</td>`; // Display process numbers on left
        for (let j = 0; j < numCols; j++) {
...
function runBankersAlgorithm() {
    ...
    // Initialize arrays to store input data
    const maxResources = [];
    const allocations = [];
    ...
    let work = available.slice(); // Make a copy of available resources
    let count = 0;
    ...
    if (safeSequence.length === numProcesses) {
        ...
        // Display available resources matrix
        ...
        document.getElementById('message').textContent = 'The system is in a safe state.';
